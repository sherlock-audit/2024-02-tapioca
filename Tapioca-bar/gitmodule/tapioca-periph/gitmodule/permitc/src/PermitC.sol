...
contract PermitC is EIP712, IPermitC {
    ...
    external
    onlyRegisteredAdvancedTypeHash(advancedPermitHash)
    returns (uint200 quantityFilled, bool isError) {
        bytes32 digest = 
    ...
    function _orderTransfer(
    ...
    ) internal returns (uint200 quantityFilled, bool isError) {
        ...
        if (orderStatus.state == ORDER_STATE_OPEN) {
            if (orderStatus.amount == 0) {
                ...
                orderStatus.amount = orderFillAmounts.orderStartAmount;
            ...
            unchecked {
                ...
                orderStatus.amount -= quantityFilled;
